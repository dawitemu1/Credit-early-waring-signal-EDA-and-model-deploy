...
st.markdown(
    """
    ### Welcome to Loan Status prediction/credit healthy Tool!
    ...
    Dive into the rich data of CBE from 2014 to 2024, interact, and uncover valuable insights for decision making!
    """
)
...
if st.sidebar.button("Predict"):
    ...
    st.sidebar.write("Prediction:", LOAN_STATUS[prediction[0]])

    # Apply model to make predictions
    prediction_proba = loaded_model.predict_proba(input_df)

    st.subheader('Prediction (credit is healthy?)')
    st.write(f"Early_warning_Signals_Loan_status: {LOAN_STATUS[prediction[0]]}")

    st.subheader('Prediction Probability')
    probability_df = pd.DataFrame(prediction_proba, columns=LOAN_STATUS)
